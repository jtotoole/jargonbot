...
get_jargon = () ->
  ...
  statement = adverbs[adverbs.length-1];
...
module.exports = (robot) ->
  ...
  robot.respond /jargon (.*)$/i, (msg) ->
    if msg.match[1] == 'me'
      msg.send get_jargon()
    else
      msg.send "please " + get_jargon() + ", @" + msg.match[1]

  robot.hear new RegExp("(^|\\W)(#{worst_offenders.join('|')})(\\z|\\W|$)", 'i'), (msg) ->
    msg.send get_jargon()
